        ...
        while self.driver.title.find('选择座位') == -1 or len(self.driver.find_elements(By.XPATH, "//span[contains(text(),'选座购买')]"))>0: # 如果跳转到了订单结算界面就算这步成功了，否则继续执行此步
            try:
                WebDriverWait(self.driver, self.total_wait_time, self.refresh_wait_time).until(
                            EC.presence_of_element_located((By.XPATH,"//*[contains(text(),'（上本）')]")))
                choiceTime_list = self.driver.find_elements(By.XPATH,"//*[contains(text(),'（上本）')]")
    ...
    def seatSelect(self):
        ...
        while self.driver.title.find('确认订单') == -1 or len(self.driver.find_elements(By.CLASS_NAME, "seat-price"))>0: 
            ...
            while True:  #当中有耗时的查找座位表动作，用true循环查到再进下一步
                ...
                for self.price in range(1,len(self.price_color_list)):
                    try:
                        ...
                        seat_list = self.driver.find_elements(By.XPATH, '//*[@class="iconfont"]' and '//li[@class!="unavailable"]' and f'//li[@style=!"color: rgb(170, 170, 170);"]')
                        # seat_list = self.driver.find_elements(By.XPATH, '//*[@class="iconfont"]' and '//li[@class!="unavailable"]' and f'//li[@style=!"color: rgb(170, 170, 170);"]')
...
if __name__ == '__main__':
    startTime = datetime.datetime(2022, 11, 9, 14, 59, 59)  #定时功能：2019-9-25 09:17:07秒开抢
    print('=> 抢票程序还未开始...')
